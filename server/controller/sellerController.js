...
class sellerController {
    ...
    async getNewOrderList(req, res, next) {
        pool.getConnection((err, conn) => {
            ...
            conn.query('select * from product_order, product where order_product_num in (select product_num from product where product_company_num = ?) and order_state = ? and product_order.order_product_num = product.product_num order by product_order.order_date', [
    ...
    async getOrderListCompany(req, res, next) {
        pool.getConnection((err, conn) => {
            ...
            conn.query(`select * from product_order, product 
            where order_product_num in (select product_num from product where product_company_num = ?) 
            and product_order.order_product_num = product.product_num`, [
                req.session.user.company_num
            ], (err, total_order_list) => {
                if (err) throw err;

                conn.query(`select p.* from 
                            (select * from product_order, product 
                            where order_product_num in (select product_num from product where product_company_num = ?) 
                            and product_order.order_product_num = product.product_num) as p
                            limit ?, 10`, [
                        req.session.user.company_num,
                        parseInt(req.params.list_seq)
                    ],
                    (err, company_orderList) => {
                        if (err) throw err;

                        var list_len = total_order_list.length

                       

                        req.total_order_count = list_len;
    ...
    async getProductList(req, res, next) {
        pool.getConnection((err, conn) => {
            ...
            conn.query(`select company_num from company where com_id = '${req.session.user.id}'`, (err, comp) => {
                if (err) throw err;
                conn.query('select * from product where product_company_num = ?',[
                    req.session.user.company_num
                ], (err, total_product_list)=>{
                    if(err) throw err;

                    conn.query(`select p.* from
                (select product_num,product_name,product_price,product_stock,product_detail 
                from product 
                where product_company_num = ?) as p
                limit ?,10`,[
                    req.session.user.company_num,
                    parseInt(req.params.product_list_seq)
                ], (err, list) => {
                    if (err) throw err;
                    conn.release();

                    req.total_list_len = total_product_list.length
                    req.p_num = list;
                    next();
                })
                })
